...
public class TrailenceConfiguration implements WebFluxConfigurer {
	...
	SecurityWebFilterChain springSecurityFilterChain(ServerHttpSecurity http, ReactiveAuthenticationManager authManager) {
		...
		.authorizeExchange(auth -> auth
			...
			.pathMatchers(HttpMethod.POST, "/api/user/v1/sendRegisterCode").permitAll()
			.pathMatchers(HttpMethod.DELETE, "/api/user/v1/sendRegisterCode").permitAll()
			.pathMatchers(HttpMethod.POST, "/api/user/v1/registerNewUser").permitAll()
			.pathMatchers(HttpMethod.DELETE, "/api/user/v1/sendDeletionCode").permitAll()
			.pathMatchers(HttpMethod.POST, "/api/kofi/v1").permitAll()
			.pathMatchers(HttpMethod.GET, "/api/donation/v1/status").permitAll()
			.pathMatchers(HttpMethod.POST, "/api/contact/v1").permitAll()
			.pathMatchers("/api/public/**").permitAll()
			.pathMatchers(HttpMethod.GET, "/api/trail-link/v1/trail/**").permitAll()
			.pathMatchers(HttpMethod.GET, "/api/trail-link/v1/photo/**").permitAll()
			.pathMatchers(HttpMethod.GET, "/api/live-group/v1").permitAll()
			.pathMatchers(HttpMethod.PUT, "/api/live-group/v1").permitAll()
