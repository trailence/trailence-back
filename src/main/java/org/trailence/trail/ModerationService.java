...
public class ModerationService {
	...
	public Flux<TrailAndPhotos> getTrailsToReview(Authentication auth) {
		return trailRepo.findTrailsToReview(auth.getPrincipal().toString())
		.flatMap(trail ->
	...
	public Mono<TrailAndPhotos> getTrailToReview(String trailUuid, String owner, Authentication auth) {
		...
		if (owner.equals(auth.getPrincipal().toString())) return Mono.error(new ForbiddenException());
	...
	public Mono<Track> getTrackFromReview(String trailUuid, String trailOwner, String trackUuid, Authentication auth) {
		if (trailOwner.toLowerCase().equals(auth.getPrincipal().toString())) return Mono.error(new ForbiddenException());
	...
	public Mono<Flux<DataBuffer>> getPhotoFileContentFromReview(String photoOwner, String photoUuid, Authentication auth) {
		if (photoOwner.toLowerCase().equals(auth.getPrincipal().toString())) return Mono.error(new ForbiddenException());
	...
	public Mono<Trail> updateTrailForReview(Trail trail, Authentication auth) {
		...
		.flatMap(entity -> 
			...
			.flatMap(collection -> {
				...
				trail.setCollectionUuid(collection.getUuid().toString());
				return trailService.updateTrailAsSuperUser(entity, trail);
			})
		);
	}
	...
	public Mono<Void> deletePhoto(String uuid, String owner, Authentication auth) {
		...
		if (email.equals(auth.getPrincipal().toString())) return Mono.error(new ForbiddenException());
	...
	public Mono<Trail> updateTrailTrack(String trailUuid, String trailOwner, Track track, Authentication auth) {
		...
		.flatMap(trail ->
			...
			.flatMap(newTrack -> {
				...
				return trailService.updateTrailAsSuperUser(trail, newDto);
			})
		);
	}
