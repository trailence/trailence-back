...
public class PublicTrailV1Controller {
	...
	public Mono<PublicTrail> getById(@PathVariable("uuid") String uuid) {
		return service.getById(uuid);
	}
	
	@GetMapping("/trailBySlug/{slug}")
	public Mono<PublicTrail> getBySlug(@PathVariable("uuid") String slug) {
		return service.getBySlug(slug);
	}
	
	@PostMapping("/trailsByIds")
	public Mono<List<PublicTrail>> getByIds(@RequestBody List<String> uuids) {
		return service.getByIds(uuids);
	}
