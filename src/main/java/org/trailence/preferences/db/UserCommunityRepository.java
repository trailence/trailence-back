...
public interface UserCommunityRepository extends ReactiveCrudRepository<UserCommunityEntity, String> {
	...
	@Modifying
	@Query("INSERT INTO user_community (email,nb_publications) VALUES (:email,1) ON CONFLICT (email) DO UPDATE SET nb_publications = user_community.nb_publications + EXCLUDED.nb_publications")
	...
	@Modifying
	@Query("INSERT INTO user_community (email,nb_comments,nb_rates) VALUES (:email,:nbComments,:nbRates) ON CONFLICT (email) DO UPDATE SET nb_comments = user_community.nb_comments + EXCLUDED.nb_comments, nb_rates = user_community.nb_rates + EXCLUDED.nb_rates")
	...
	@Query("SELECT u.email as email, c.public_uuid as public_id, pref.alias as alias, avatar.public_uuid as avatar, COALESCE(c.nb_publications, 0) as nb_publications, COALESCE(c.nb_comments, 0) as nb_comments, COALESCE(c.nb_rates, 0) as nb_rates FROM users u LEFT JOIN user_preferences pref ON pref.email = u.email LEFT JOIN user_avatar avatar ON avatar.email = pref.email AND avatar.current_file_id IS NOT NULL AND avatar.current_public = TRUE LEFT JOIN user_community c ON c.email = pref.email WHERE u.email IN (:emails)")
